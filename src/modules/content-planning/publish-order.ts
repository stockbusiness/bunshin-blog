 ...
 * ## 上限はブログの設定を使う
 *
 * SPEC 2.2 の5本は**絶対の上限**で、ブログごとの `weeklyPublishCap`
 * （3〜5本。B-5・Q-036）はそれ以下。**低いほうを使う。**
 *
 * 上限が5本より低いと、収益記事が2週で収まらないことがある（案件3件なら
 * 7本）。そのときは**3週目以降にもかかる** — SPEC 9.2.7 の「1〜2週目」は
 * 上限いっぱいを前提にした書き方で、**守るべきは「収益記事が先行する」ほう**
...
/** 1週あたりの絶対上限（SPEC 2.2。2026-08-12 に4→5・Q-036） */
export const ABSOLUTE_WEEKLY_CAP = 5;
