 ...
 *
 * **リクエストの中で動かす前提**（Cloud Run・Q-045）。常駐ワーカーを
 * 持てないため、次の2点が設計の中心になる。
 *
 * - **締め切りを持って抜ける。** リクエストには上限があり、超えると
 *   途中で殺される。残ったジョブは次の起動に任せる
 * - **1件あたりの時間も区切る。** 1件が長引くと、そのジョブごと
 *   リクエストが殺され、行が `RUNNING` のまま残る（回収は `reclaimStuckJobs`）
 *
