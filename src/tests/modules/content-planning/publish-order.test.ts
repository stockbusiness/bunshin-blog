 ...
 *
 * 完了条件「**収益記事が先行し、集客記事が週の上限を超えない**」。
 * 上限は3〜5本（SPEC 2.2・Q-036。2026-08-12 に4本固定から改めた）。
 *
 * DBもAIも触らない純粋な処理。
 */
...
describe('週の上限を超えない（完了条件）', () => {
  it.each([[3], [4], [5]])('上限%s本を超えない', (weeklyCap) => {
   ...
   *
   * **0も受け付けない。** 公開を止めたブログ（G-8）の構成表を
   * 作ろうとしていることになり、そもそも呼ぶ側が誤っている
   */
  ...
  it('絶対の上限は5本', () => {
    expect(ABSOLUTE_WEEKLY_CAP).toBe(5);
  });
});
