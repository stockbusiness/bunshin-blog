...
| 2 | **Google Cloud SQL for PostgreSQL（東京）** | **提供元は決まっている**（Q-028）。2.1 |
...
**接続プールはいま挟まない**（Q-028）。Vercel は関数が同時に何本も
立ち上がるため、利用者が増えると接続数を使い切るが、**30ブログでは
起きない。** 起きたら PgBouncer か Cloud SQL Auth Proxy を挟む —
**これは「足す」変更であって「移す」変更ではない。**
