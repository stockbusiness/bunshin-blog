...
**Vercel は使わない**（Q-045）。Cloud SQL に公開IPで繋ぐには承認済み
ネットワークに実質 `0.0.0.0/0` を入れることになり、**DBの守りが
パスワード1つだけ**になるため。
...
**アプリは Unix ソケットで繋ぐ**（Q-045）。Cloud Run が Cloud SQL Auth
Proxy を内側で動かすので、**インスタンスに公開IPは要らない。**

「接続」→ **パブリックIP を無効**、**プライベートIP も不要**。

**承認済みネットワークに `0.0.0.0/0` を入れない。** 入れると、DBの守りが
**パスワード1つだけ**になる。中には `line_user_id`・記事本文・
**暗号化した WordPress の認証情報**がある（SPEC 14.2）。

**手元から `psql` や `prisma migrate deploy` を流すときだけ、
