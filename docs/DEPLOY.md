...
**まとめて入れるなら、ASPのCSVを取り込みます。**

| | すること |
|---|---|
| 1 | ASPの管理画面から「提携プログラム一覧」をCSVで書き出す |
| 2 | `/admin/offer-catalog` の「ASPのCSVから取り込む」へ上げる |
| 3 | 列の対応を確かめる（AIが推測したものを直せます） |
| 4 | 残った候補から選んで「下書きとして登録」 |

**報酬額・成果条件・提携状態で自動的に絞り込みます**（SPEC 9.2.3）。
**何件がどの理由で落ちたかが画面に出ます。**

**取り込んだものは「下書き」で、事実は空です。** 1件ずつ
「このページから読み取る」で事実を入れてから「選べる」にしてください。
...
**運営のカタログは「どの案件を紹介してよいか」の一覧であって、
提携そのものではない**（Q-057、2026-08-17 決定）。

| | 誰がやるか |
|---|---|
| 案件をカタログに載せる・事実を確かめる | **運営** |
| ASPと契約し、案件に提携申請する | **モニター本人** |
| アフィリエイトリンクを発行して貼る | **モニター本人**（段8で1つだけ打つ） |
| 成果レポートを出す | **モニター本人**（CSVで上げる。MANUAL 3章） |

**運営が代わりに成果を入れることはできない。** モニターを迎える前に、
**ASPのアカウントを本人が作っていることを確かめる。**
...
| 9 | **成果CSVが読める** | `/liff/results` にASPの成果レポートを上げると、週ごとの成果が表になって出る（5.0） |
...
| 「8週間継続率」の継続の定義（Q-043） | **決定済み**（2026-08-17）。`/admin/monitor-activity` に出る |
| 「公開後に誤りを見つけたら `fact_issues` に入れる」運用の約束 | **決定済み**（2026-08-17）。`/admin/fact-issues` に出る |
| 最初のジャンルと案件数 | **決定済み**（2026-08-17）。**登録はこれから** |

**意思決定は終わった。** 残るのは実装と、下の Go/No-Go である。

### 運営が毎週やること

**決めた運用（`docs/VALIDATION.md` 2.2）。守らないと数字が空になる。**

| | すること |
|---|---|
| 1 | モニターからLINEで届いた指摘を `/admin/fact-issues` へ登録する |
| 2 | **公開済み記事から10件を抜き取って確かめる**（負荷が高くても最低5件） |
| 3 | 見つけた誤りを同じ場所へ登録する |
| 4 | **「確認した」ことを記録する**（`POST /api/admin/fact-reviews`） |

抜き取りの優先順位：収益記事 → 数値・料金・条件を多く含む記事 →
最近更新された案件を扱う記事 → 修正依頼があった記事 → 無作為。

**重大な誤りは週次を待たず、見つけた時点で登録する。**

**4を飛ばさない。** 飛ばすと、`fact_issues` が空のときに
**「誤りが無かった」のか「確かめていない」のか**が分からなくなる。

**`/admin/fact-issues` に「今週はまだ確認していません」と出る。**
消えたら、その週は済んでいる。

### 検証を始めてよいか（Go/No-Go）

**意思決定が終わっただけでは始められない。**
**実LINE・実AI・実WordPress**で通ることを確かめる。
...
- [ ] 管理者ログインのメールが届く
- [ ] `/admin/settings` の必須設定がすべて入っている

#### LINE

- [ ] 実アカウントで友だち追加できる
- [ ] Webhook 検証が 200 を返す
- [ ] リッチメニューが出て、押すと LIFF が開く
- [ ] 記事提案の通知が実際に届く
- [ ] 承認・修正・見送りが実際に保存される
...
- [ ] 下書き投稿と、投稿後の同期が成功する
- [ ] **利用者が編集した記事を上書きしない**
- [ ] `/go/` または DIRECT のリンクが実際に動く

#### 案件・ジャンル

- [ ] 通せるジャンルが2件登録済み
- [ ] 各ジャンル10〜15件の選べる案件がある
- [ ] **事実確認済みのものだけが出る**
- [ ] カタログから案件を選べる
- [ ] 本人のアフィリエイトリンクを登録できる
...
```
LINE登録 → 同意 → 分身 → ブログ → WordPress接続 → 案件 → ジャンル
→ 構成表 → 記事 → LINE承認 → WordPress下書き → 公開
→ Search Console 取得 → 成果CSV登録
```

**外部10名を募集する前に、ここまで1人で通す。**
