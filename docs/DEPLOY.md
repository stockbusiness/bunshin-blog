...
`/admin/offer-catalog` から入れる。**モニターを迎える前に要る** —
案件が1件も無いと、モニターは段7のジャンル審査を通れない（案件0件は停止条件）。

| | すること |
|---|---|
| 1 | ASPと契約し、紹介したい案件を選ぶ |
| 2 | 「紹介先のページ」を入れて「このページから読み取る」 |
| 3 | **読み取った事実を必ず確かめる**（ここが全モニターの記事に載る） |
| 4 | ASPの名前・報酬額を入れる（LPからは読み取れない） |
| 5 | 「ASPの規約に関わる設定」を開き、リンクの出し方を決める |
| 6 | 状態を「選べる」にして登録 |

**事実を入れないと「選べる」にできません。** 記事に書ける数値は案件の
「事実」から取るため（SPEC 9.6）。

**リンクの出し方は既定が「そのまま貼る」です。** 規約を確かめたものだけ
`/go/` を経由させてください。**誤ると成果が無効になります**（Q-001）。

### 5.1 LINEのメニュー（リッチメニュー）

`/admin/rich-menu` から作る。**モニターを迎える前に要る** —
`docs/MANUAL.md` が「トーク画面の下にあるメニューから開く」と書いている。

**先に `LIFF_BASE_URL` を入れておく**（3.3）。行き先の組み立てに使うので、
未設定だと画面がそう言って先へ進めない。

| | すること |
|---|---|
| 1 | 「押す場所の型」を選ぶ（2段3列など） |
| 2 | 画像を上げる。**PNG/JPEG・1MB以下・枠と同じ縦横比** |
| 3 | 升目ごとに行き先を選ぶ |
| 4 | 「保存する」 |
| 5 | 「LINEへ出す」 |
| 6 | 「いま出ているものを確かめる」 |

**保存しただけでは誰にも出ない。** 「LINEへ出す」を押して初めて出る。

**縦横比が枠と違う画像は受け取らない。** LINEは画像を枠へ引き伸ばすので、
比が違うと**升目と、指で押す場所がずれる。**

#### 先に知っておくこと

**ここで作ると、LINE公式アカウントマネージャーの画面からは
編集できなくなる**（LINEの仕様）。**両方から触る運用にはできない。**

公式アカウントマネージャーで既にリッチメニューを作っている場合は、
**そちらを消してからここで作り直す。**

#### LIFF のIDを変えたとき

**リッチメニューのボタンは全部が黙って壊れる**（押しても何も起きない）。
`/admin/rich-menu` の**「行き先をいまの設定に合わせる」**を押し、
保存して「LINEへ出す」をもう一度押す。
...
| 7 | **メニューが出ている** | Bot のトーク画面の下にメニューが出て、押すと LIFF が開く（5.1） |
| 8 | **案件カタログに中身がある** | `/admin/offer-catalog` に「選べる」案件が並んでいる（5.0） |
