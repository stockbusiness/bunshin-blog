...
| つなぎ方 | **Unix ソケット `/cloudsql/<接続名>`。パブリックIPを使わない** |
...
**Cloud Run にした理由は、DBを公開範囲から外せること。** 接続元のIPが固定されない実行先だと、Cloud SQL の承認済みネットワークに実質 `0.0.0.0/0` を入れることになり、**守りがデータベースのパスワード1つだけ**になる（Q-045）。Cloud Run は Cloud SQL Auth Proxy を内側で動かし、**公開IPを持たないインスタンスへ繋げる。**
...
**44タスク／75 が完了**（2026-08-08）。**Phase A・B・C は全件終わった。** **Phase D は全件終わった**（D-7 は D-7a／D-7b に分けて両方とも完了。Q-015 は解決済み）。Phase E は基盤（E-1〜E-3）と費用（E-14・E-15）まで。**残る E-4〜E-13 は実際にAIを呼ぶため、`ANTHROPIC_API_KEY` が要る。**
...
| `ai-costs` | AI費用の記録と集計（E-14）、予算通知（E-15）。**Phase 0 は超過しても生成を止めない** |

`experiments` `content-planning` `approvals` `audit` `line` は未実装（空ディレクトリ）。
...
| `ai/` | プロバイダー抽象化とモデルルーティング（E-3）。**モデル名は呼び出し側に出さない** |
