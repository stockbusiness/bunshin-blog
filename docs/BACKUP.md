...
運用先は **Google Cloud SQL for PostgreSQL（東京）**（Q-028、2026-08-11）。

| 設定 | 値 | 理由 |
|---|---|---|
| 自動バックアップ | **有効。保持7日以上** | 「先週おかしくなった」に戻れる |
| ポイントインタイムリカバリ | **有効。7日以上遡れること** | **誤った `UPDATE` を1回流しただけで、その日の承認が全部消えるのを防ぐ**。承認の記録は実験の一次データ（SPEC 1.2） |
| 取得時刻 | **JST 03:00〜04:00** | 提案が届くのは朝（SPEC 8.3）、投稿ジョブもその後。**最も動きが少ない時間帯** |

**PITR が標準で含まれることが選定理由。** 別料金のアドオンにすると、規模が変わるたびに費用の判断が要る。

**契約時に確かめること：** 上の3つが実際にその値で設定されていること。**「有効にしたつもり」を残さない**（4章の復旧確認と同じ趣旨）。
