...
**この判断は、こちらで調べられない事実に依存する。** 決定をお願いしたいのは次の点である。
...
#### ASPによって規約が違う場合の案（2026-08-07 追記）

「許可しているASPと、していないASPが混在する」という前提が確認された。**この前提では、方式を1つに決めること自体が誤り**である。以下を提案する。
...
D-8 の完了条件「記事内リンクが**自前URL経由**になり、クリックが記録される」は、この案では**「リンク方式に従って組み立てられ、`REDIRECT` の案件でクリックが記録される」**へ改める必要がある。

#### 補足

- SPEC 11.5 は **Phase 0 の収益を手入力で許可**している。つまり (c)(d) でも収益の記録は成立する。失われるのは「クリック数」であり「収益」ではない
- (c)(d) を選ぶ場合、`affiliate_links` と `link_clicks` は Phase 0 では使わないテーブルになる。**DATA_MODEL 6章に「方式が確定するまでこの2テーブルは確定版ではない」と既に書いてある**ため、削除ではなく未使用のまま残す判断もできる
- **SPEC 20.1 は本来「Phase A着手前」に決めるとしていた項目**である。Phase A・B を終えた現時点でも Phase C の実装には影響していないが、D-8 と E-10 の前には必要になる
