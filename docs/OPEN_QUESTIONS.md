...
- 状態：**未解決。B-5 着手前に決定が必要**

推奨は (a)。停止条件はジャンル選定の中核（SPEC 9.2.2）であり、設定画面から素通りできる経路を作ると審査の意味が無くなる。
...
推奨は (a)。1日1件という制限は「モニターに届く通知の総量」の話であり、ブログごとに曜日と時刻を分けると3ブログで最大3倍の通知が出る設計になって SPEC 2.x の意図と食い違う。
...
推奨は (b)。この比率を読む処理がまだ無く、先に入力欄だけ作ると、E-9 の実装時に意味が確定して作り直しになる。あわせて **`article_ratio.revenue` / `traffic` は利用者に編集させない**（算出値のため）。設定画面で編集してよいのは `weeklyPublishCap`（投稿頻度、上限4・SPEC 2.2）に限る。
