...
- 状態：**未解決。B-7 着手前に決定が必要**

推奨は **(a)**。

- SPEC 3.2 が挙げる3つ目「ADMINロールによる制御」に該当し、廃止対象の Basic 認証ではない
- Phase 0 の管理者は1〜2名（SPEC 3.1）。そのために外部サービスとセッション機構をもう1系統持つ割に合わない
- (c) はスキーマ変更とメール基盤の選定が前提になり、B-6 の完了条件から離れる
- **後から差し替えられる。** B-6 の `requireAdmin` はセッションからユーザーを解決した後の判定であり、認証手段が変わっても影響を受けない

(a) を採る場合の注意点も挙げておく。

- 管理者も LINE アカウントが必要になる
- PC から使う場合、LIFF の URL をブラウザで開いてセッションCookieを得る手順になる
- ADMIN への昇格は SQL（`update users set role = 'ADMIN' where ...`）。Phase 0 では専用UIを作らない方針（SPEC 10.3・Q-008 と同じ扱い）
