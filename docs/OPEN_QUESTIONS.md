...
**D-9 では実装しない。** D-9 の完了条件は `link_mode` の追加のみであり、サブIDの形が決まらないうちに列を足すと、決まったときに作り直しになる。**スキーマ変更は単独タスク**（DATA_MODEL 9章）なので、(a) を選ぶ場合は D-1 の前にもう1つ小さなマイグレーションを挟む。

**確認をお願いしたいこと。**

1. 利用予定のASPは何か（A8.net・もしもアフィリエイト・afb・アクセストレード 等）
2. それぞれサブID（サブアフィリエイトID・s1・argument など名称は様々）を使えるか
3. 使える場合、パラメータ名と文字数・使える文字の制限

**1つでも分かれば (a) で進められる。** 全ASPを揃える必要は無く、`NULL` の案件はサブIDを付けないだけで動く。
