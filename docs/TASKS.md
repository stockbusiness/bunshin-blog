...
- **仕様と実装の食い違いを見つけたら、その場で `docs/OPEN_QUESTIONS.md` へ記録する。** 直すかどうかは別の判断で、**見つけた人が残す**。残さないと、次に気づくのはずっと後になる（Q-038 の食い違いは C-9 の実装時点で生まれたが、見つかったのは4日後だった）
- **`docs/SPEC.md` は実装タスクで書き換えない。** 仕様が実態と違っていても、記録するところまで。**利用者が明示的に決めたときだけ、docs のみの PR で反映する**（Q-036・Q-038・Q-040・Q-041 はいずれもこの形で反映した）
...
**着手を止めている論点は無い。** SPEC 20.1 の2件（Q-001・Q-002）に加え、Q-005（Search Console の暦日）と Q-014（サブIDの付け方）も決着した。

**未決の論点はある。** 数と中身は `docs/OPEN_QUESTIONS.md` が正で、**ここに写しを置かない**（写しは必ず古びる。この節も「未解決の論点は無い（2026-08-08 時点）」と書いたまま4件が積み上がっていた）。

**検証が始まる前に決めなければならないもの**だけは、`docs/VALIDATION.md` 4章に集めてある。
...
**Phase A〜H で作った部品が、実際に動き出せる形になっているかを埋める。** 棚卸し（2026-08-12）で、**完了条件は満たしているのに繋がっていない**箇所が見つかった。各タスクの完了条件が「保存される」「判定できる」までで、**誰がそれを呼ぶか**を含んでいなかったため。

| ID | タスク | 依存 | 完了条件 | 主な変更先 |
|---|---|---|---|---|
| I-1 | 日次ジョブを積む経路 | E-1, G-2, G-3, G-6, H-3 | 検索データ取得・インデックス確認・日次集計・リンク確認が、**cron の1本から1日1回積まれる**。同じ日に二度積まれない。1利用者の失敗で他が止まらない | `src/app/api/jobs/run/` `src/modules/line/` |
| I-2 | 提案の選定と通知を繋ぐ | F-1, F-2, F-3b, I-1 | `PROPOSAL_SELECTION` にハンドラがあり、**選定 → 通知が日次で動く**。通知の時間帯（F-3b）と1日の件数（F-3）が効く | `src/app/api/jobs/run/` `src/modules/approvals/` |
| I-3 | 案件・バナー・WordPress同期のHTTP入口 | D-1, D-3, C-5 | SPEC 13.3〜13.5 の入口があり、**オンボーディング STEP 8（案件登録）が画面から完了できる** | `src/app/api/` `src/app/liff/` |
| I-4 | 記事の生成と投稿を積む経路 | I-1, E-9, E-10 | 構成表の公開予定（C-9 の曜日・時刻）に従って `ARTICLE_GENERATION` が積まれる。**週の上限を超えない** | `src/app/api/jobs/run/` |
...
| I-7 | デプロイ手順書 | — | **実行先**の設定・環境変数・cron・DB・`ENCRYPTION_KEY` の作り方が文書化されている | `docs/` |
| I-8 | WordPress接続の認可フロー | C-2 | モニターが**アプリケーションパスワードを手で発行せず**にブログをつなげる（SPEC 7.1・v2.3）。**手で貼る経路も残る** | `src/app/api/blogs/[blogId]/wordpress/` `src/modules/wordpress/` |
...
| I-10 | 初期構成表を積む経路 | E-9, H-2a | オンボーディングが終わったブログの `PLAN_GENERATION` が積まれる。**整っていないブログは飛ばす** | `src/app/api/onboarding/` |
...
**記事が検索で見つかっても、開いた先が読みにくければ読まれない**（Q-041 の決定・2026-08-12）。

**見た目を揃えない。** 30ブログが同じテーマだと、これまで避けてきた「同一運営者の痕跡」がそのまま出る（Q-001 の再決定・D-12・Q-040）。**揃えるのは質だけで、見た目は散らばったままにする。**

| ID | タスク | 依存 | 完了条件 | 主な変更先 |
|---|---|---|---|---|
| J-1 | 本文に目次・`alt`・表の見出しを入れる | E-11, I-5 | 生成された本文HTMLに**目次**があり、画像に `alt`、表に `<th>` が付く。**テーマを問わず効く** | `src/modules/content-generation/` |
| J-2 | 表示品質の測定と通知 | H-3, J-1 | 公開済みの記事を**実際に取得して**測り、満たさなければリンク切れと同じ器で通知される | `src/modules/line/alerts.ts` `src/modules/analytics/` |
| J-3 | 推奨テーマの手引き | — | `docs/MANUAL.md` に**3〜5個**の候補と選び方がある。**1つに絞らない** | `docs/MANUAL.md` |
| J-4 | 記事の厚みを測る | E-12, E-13 | 薄い記事に**リスクフラグが立ち、承認画面で見える**。**止めない**（作り直す経路が無いため） | `src/modules/content-generation/` |
| J-5 | モニターの反応を測る | F-2, F-6 | 直近14日の**送れた提案**に対する反応の割合が、管理画面でモニターごとに見える。**送れていない人と反応が無い人を分ける** | `src/modules/approvals/` `src/app/admin/` |
| J-6 | 90日検証の集計軸を決める | — | SPEC 16.2 の目標**ひとつひとつ**について、いま計算できるかが `docs/VALIDATION.md` にある。**足りないものが特定されている** | `docs/` |
| J-7-schema | 事実誤認を記録する表 | — | `fact_issues` が作られ、**説明が空なら登録できない**（Q-044） | `prisma/` |
| J-7 | 事実誤認の記録と集計 | J-7-schema | ADMIN が誤りを登録でき、**公開前に捕まえた割合**が出る（SPEC 16.2） | `src/modules/` `src/app/admin/` |

**J-1 を先にする理由。** 本文HTMLは Bunshin が作っているので、**モニターに何も頼まずに効く。** J-2 の測定は「直っているか」を見る仕組みで、直す手段（J-1）が無いうちに作ると**指摘だけが出て直せない。**

**J-2 で `safeFetch` を必ず通す**（SPEC 14.3・C-7）。測る先は利用者が登録したURLである。
