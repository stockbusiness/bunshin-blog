...
以下が未決のままPhase Cに入ってはならない（SPEC 20.1）。

- アフィリエイトリンクのリダイレクト方式 → **D-8の前提**（OPEN_QUESTIONS Q-001）
- ドメイン・サーバー費用の負担者 → **H-1の前提**（OPEN_QUESTIONS Q-002）

**実際に止まるのは D-8・E-10（Q-001）と H-1（Q-002）である。** Phase C（C-1〜C-6）は WordPress 接続とテナント越境の検証であり、どちらの決定にも依存しない。Phase B 完了時点でこの2件は未決のままだが、**C-1 の着手は妨げられない**。

Q-001 は**ASPの規約に依存する判断**である。**許可するASPとしないASPが混在する**ことが確認されたため、方式を1つに決めず**案件ごとに切り替える案**を OPEN_QUESTIONS Q-001 に整理した。決定は D-1 着手前に必要（案件登録時にリンクの組み立て規則が決まるため）。
