...
| D-11-schema | `affiliate_links.blog_id` と複合外部キー | A-2, E-6 | マイグレーションのみ。記事と案件が同じブログであることをDBが強制する（Q-020） | `prisma/` |
| D-11 | リンク発行で `blog_id` を埋める | D-11-schema | 他人の記事IDを紐づけたリンクが作れない | `src/modules/affiliate/` |
...
**タスク総数：84**（当初75。スキーマ変更を単独タスクにしたぶんと、
