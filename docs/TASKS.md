...
| G-8 | インデックス率による公開ペース調整 | G-3, C-9 | ブログ単位で2週間ごとに判定し、`weekly_post_cap` を上下させる（80%以上→+1・上限5、50%未満→0にしてADMINへ通知）。**公開後14日未満の記事は母数に含めない**。調整の履歴を管理画面で確認できる（作業指示書 W-8）。**範囲を 3〜5 へ広げるコード側の変更もここで行う**（`WEEKLY_PUBLISH_CAP_MIN`/`MAX`・設定画面の選択肢・DATA_MODEL 4章の制約7。Q-036 は SPEC 側を 2026-08-12 に改訂済み） | `src/modules/analytics/` `src/modules/blogs/` `src/app/admin/` `src/app/liff/` |
