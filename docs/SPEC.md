...
改訂：**2026-08-12（v2.2 → v2.3）**。WordPress の接続に**認可フロー**を加える（7.1・6.1 段6。OPEN_QUESTIONS Q-040）。**アプリケーションパスワードを手で発行して貼り付ける作業をなくす**（オンボーディングで最も脱落しやすい箇所であるため）。手で貼る経路は**残す**（認可フローが使えないサイトがあるため）。

改訂：**2026-08-12（v2.3 → v2.4）**。初期記事数を「必ず30記事」から **28〜34記事**へ改める（9.3。OPEN_QUESTIONS Q-038）。**C-9 が実装済みの `blogs.initial_article_count` と食い違っていた。** 散らす理由（同一運営者の痕跡を残さない）のほうが後から取り返せないため、仕様のほうを改める。
...
- **WordPressで承認する**（認可フロー。v2.3。既定の経路）

- WordPressユーザー名（**手で貼る場合のみ**）

- アプリケーションパスワード（**手で貼る場合のみ**）
...
**発行は WordPress の認可フローで行う**（v2.3）。モニターは自分の
WordPress で「承認」を1回押すだけで、**発行・コピー・貼り付けを行わない。**

```
https://<ブログ>/wp-admin/authorize-application.php
  ?app_name=BUNSHIN BLOG&success_url=<Bunshin の戻り先>&state=<署名付き>
```

**手で貼る経路も残す。** 認可フローは **HTTPS のサイトでしか使えず**、
WordPress 5.6 より前や、`wp-admin` を独自の場所へ移したサイトでは
開けない。**片方しか無いと、そこで詰まった人が先へ進めない。**

**戻りのURLにパスワードが載る。** WordPress の仕様で、`success_url` に
`user_login` と `password` が**クエリとして**付く。受け取ったら
**そのまま保存し、クエリを落としたURLへ即座に転送する**
（履歴・Referer・アクセスログに残る時間を短くする）。**この値を
ログへ出さない**（14.2）。
...
10記事のみでは検索順位が付かず、流入・収益の検証が成立しない。初期構成表は **28〜34記事**で生成する（v2.4。OPEN_QUESTIONS Q-038）。

**本数はブログごとに散らす**（`blogs.initial_article_count`）。30ブログが揃って同じ本数で立ち上がると、**外から並べたときに同じ仕組みだと分かる**（同一運営者による大量サイトの痕跡を残さない・W-8）。

**下限を28本にしたのは、10記事では足りないという上記の懸念に触れないため。** 検証の成立に必要な本数は保ったまま、揃わないようにする。
